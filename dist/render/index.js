...
function renderUsageLine(ctx) {
    ...
    const primaryResetPart = primaryReset && usage.metric === 'weekly_percent'
    ...
    let monthlySidePart = '';
    if (usage.metric === 'weekly_percent' &&
        typeof usage.used === 'number' &&
        typeof usage.limit === 'number' &&
        usage.limit > 0) {
        const mPct = Math.round((usage.used / usage.limit) * 100);
        monthlySidePart = dim(` · ${t('label.monthly')} ${mPct}% (${formatCreditAmount(usage.used)}/${formatCreditAmount(usage.limit)})`);
    }
    // Weekly window metadata (when bar is monthly and weekly % is gone)
    let weekPart = '';
    if (usage.weekEnd) {
        const weekIsPrimary = usage.metric === 'weekly_percent' ||
...
export function visualRowCount(text, columns) {
