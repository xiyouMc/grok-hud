...
function toolTarget(name, rawInput) {
    if (!rawInput)
        return undefined;
    const candidates = [
        rawInput.path,
    ...
    if (typeof rawInput.tool_name === 'string')
        return rawInput.tool_name;
    return name;
}
...
function normalizeToolName(title, metaName) {
    if (metaName && metaName.trim())
        return metaName.trim();
    if (!title)
        return 'tool';
    // "Execute `ls -la ...`" → keep as shell-ish label
    if (/^Execute\s+/i.test(title))
 ...
 */
export function parseRecentTools(updatesPath, limit = 12) {
    ...
    for (const line of lines) {
        ...
        const statusRaw = raw.params?._meta?.updateParams?.status ||
            update.status ||
    ...
    list.sort((a, b) => {
        const rank = (s) => (s === 'running' || s === 'pending' ? 0 : 1);
